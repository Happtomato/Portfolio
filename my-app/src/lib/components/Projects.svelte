...
<section id="projects" class="bg-deepblue px-6 sm:px-10 py-20">
    <div class="animate-fadeIn mx-auto max-w-7xl text-center">
        <h2 class="mb-12 bg-gradient-to-r from-accentPink via-accentOrange to-accentYellow bg-clip-text text-4xl lg:text-5xl font-bold text-transparent">
            Projects
        </h2>

        <div class="relative h-[480px] md:h-[560px] lg:h-[600px]" on:touchstart={handleTouchStart} on:touchend={handleTouchEnd}>
            {#key current}
                    ...
                    >
                        ...
                        >
                            <img
                                    ...
                                    width="800"
                                    height="600"
                                    class="h-auto max-h-[260px] md:max-h-[300px] lg:max-h-[350px] w-auto max-w-full rounded-xl object-contain shadow-lg"
                            />
                            ...
                            <div class="flex-1 text-left">
                                ...
                                <div class="flex flex-col gap-3 sm:flex-row">
                                    {#if projects[current].link}
                                        <a href={projects[current].link} target="_blank" class="rounded-lg bg-accentPink px-4 py-2 font-semibold text-deepblue transition hover:bg-accentOrange">
                                            View Project
                                        </a>
                                    {/if}
                                    {#if projects[current].github}
                                        <a href={projects[current].github} target="_blank" class="inline-flex items-center gap-2 rounded-lg bg-accentOrange px-4 py-2 font-semibold text-deepblue transition hover:bg-accentPink">
            ...
            <button on:click={prev} aria-label="Previous project" class="absolute top-1/2 -left-7 md:-left-10 -translate-y-1/2 transform rounded-full bg-accentPink p-2 md:p-3 text-deepblue shadow transition-transform duration-200 hover:bg-accentOrange" class:animate-bumpLeft={leftKick}>
            ...
            <button on:click={next} aria-label="Next project" class="absolute top-1/2 -right-7 md:-right-10 -translate-y-1/2 transform rounded-full bg-accentPink p-2 md:p-3 text-deepblue shadow transition-transform duration-200 hover:bg-accentOrange" class:animate-bumpRight={rightKick}>
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 md:h-6 md:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="mt-8 flex justify-center gap-2">
            {#each projects as _, i}
                <button on:click={() => (current = i)} aria-label={`Go to project ${i + 1}`} class="h-3 w-3 rounded-full transition" class:bg-accentPink={i === current} class:bg-accentOrange={i !== current}></button>
            {/each}
        </div>
    </div>
</section>
