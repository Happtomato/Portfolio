<script lang="ts">
	...
	const projects: Project[] = [
		...
		},
		{
			...
			description:
				'Final apprentices hip project integrating an AI-powered support chatbot with Svelte frontend and Flask/Azure backend.',
			image: IPAImage,
...
<section id="projects" class="bg-deepblue px-10 py-20">
	<div class="animate-fadeIn mx-auto max-w-5xl text-center">
		<h2
			class="mb-10 bg-gradient-to-r from-accentPink via-accentOrange to-accentYellow bg-clip-text text-4xl font-bold text-transparent"
		>
			Projects
		</h2>

		<div
			class="relative h-[460px] md:h-[380px]"
		...
		>
			{#key current}
				{#if projects && projects.length}
					...
					>
						<div
							class="bg-opacity-100 flex w-full flex-col items-center gap-6 rounded-2xl p-8 shadow-xl md:flex-row"
						...
						>
							<img
								...
								width="800"
								height="600"
								class="h-auto max-h-[320px] w-auto max-w-full rounded-xl object-contain shadow-lg md:max-h-[300px]"
							/>
							<div class="flex-1 text-left">
								<h3 class="mb-2 text-2xl font-semibold">{projects[current].title}</h3>
			...
			<button
				...
				class="absolute top-1/2 -left-6 -translate-y-1/2 transform rounded-full bg-accentPink p-2 text-deepblue shadow transition-transform duration-200 hover:bg-accentOrange"
			...
			<button
				...
				class="absolute top-1/2 -right-6 -translate-y-1/2 transform rounded-full bg-accentPink p-2 text-deepblue shadow transition-transform duration-200 hover:bg-accentOrange"
